...
```{r}
# newRowCounter = padModel(length(numericDates)*length(namesOfWomensTeams)*length(namesOfWomensTeams))
# #Women
...
# }
# cleanUpModel()
# newRowCounter = padModel(length(numericDates)*length(namesOfMensTeams)*length(namesOfMensTeams))
# #Men
...
# Check if the LP model has been correctly set up
solve(LP)
...
```{r}
# Unique Opponent Constraint
newRowCounter = padModel(numberOfRowsToAdd = length(namesOfMensTeams) * length(namesOfMensTeams))

for(t1 in namesOfMensTeams){
  for(t2 in setdiff(namesOfMensTeams, t1)){
    regexList = c(
      paste("^x", t1, t2, ".*", ".*", "Men", sep = "\\."),
      paste("^x", t2, t1, ".*", ".*", "Men", sep = "\\.")
    )
    valueList = c(1, 1)
    newIneq = "="
    newRhs = 1
...
newRowCounter = padModel(numberOfRowsToAdd = length(namesOfWomensTeams) * length(namesOfWomensTeams))

for(t1 in namesOfWomensTeams){
  for(t2 in setdiff(namesOfWomensTeams, t1)){
    regexList = c(
      paste("^x", t1, t2, ".*", ".*", "Women", sep = "\\."),
      paste("^x", t2, t1, ".*", ".*", "Women", sep = "\\.")
    )
    valueList = c(1, 1)
    newIneq = "="
    newRhs = 1
